...
export const departmentRepository = {
  ...
  async getById(id: string): Promise<DepartmentRow | null> {
    return db.query('SELECT * FROM departments WHERE id = $1')
      .execute(id);
  },
  
  async create(dept: Omit<DepartmentRow, 'id' | 'created_at'>): Promise<DepartmentRow> {
    return db.query('INSERT INTO departments (name, display_name, icon, description, color) VALUES ($1, $2, $3, $4, $5) RETURNING *')
...
export const employeeRepository = {
  async getByAuthId(authId: string): Promise<EmployeeRow | null> {
    return db.query('SELECT * FROM employees WHERE auth_id = $1')
      .execute(authId);
  },
  
  async getByDepartment(deptId: string): Promise<EmployeeRow[]> {
    return db.query('SELECT * FROM employees WHERE department_id = $1')
      .execute(deptId);
  },
  
  async updateRole(authId: string, newRole: string): Promise<void> {
    await db.query('UPDATE employees SET role = $1 WHERE auth_id = $2')
      .execute(newRole, authId);
  }
};
...
export const machineRepository = {
  async getActiveByDepartment(deptId: string): Promise<MachineRow[]> {
    return db.query('SELECT * FROM machines WHERE department_id = $1 AND active = true')
      .execute(deptId);
  },
  
  async getById(machineId: string): Promise<MachineRow | null> {
    return db.query('SELECT * FROM machines WHERE id = $1')
      .execute(machineId);
  }
};
...
export const dailyLogRepository = {
  async getByDateRange(startDate: string, endDate: string): Promise<DailyLogRow[]> {
    return db.query('SELECT * FROM daily_logs WHERE log_date BETWEEN $1 AND $2')
      .execute(startDate, endDate);
  },
  
  async getByDepartment(deptId: string): Promise<DailyLogRow[]> {
    return db.query('SELECT * FROM daily_logs WHERE department_id = $1')
      .execute(deptId);
  }
};
...
export const machineHoursRepository = {
  async getHoursByMachine(machineId: string): Promise<MachineHoursRow[]> {
    return db.query('SELECT * FROM machine_hours WHERE machine_id = $1')
      .execute(machineId);
  },
  
  async getByDailyLog(dailyLogId: string): Promise<MachineHoursRow[]> {
    return db.query('SELECT * FROM machine_hours WHERE daily_log_id = $1')
      .execute(dailyLogId);
  }
};